...
# Endpoint para processar a imagem, vamos receber uma imagem e retornar um resumo da image, por exemplo, se for uma imagem de um relatorio, vamos retornar um resumo do relatorio
def extract_text_from_image(image_data: bytes) -> str:
    reader = easyocr.Reader(['pt'])  # Define o idioma como português
    image = image_data
...
async def process_image(file: UploadFile = File(...)):
    try:
        # Lê a imagem do arquivo carregado
        ...
        # Retorna erro se falhar ao processar a imagem
        logging.error("Processing error: %s", str(e))
