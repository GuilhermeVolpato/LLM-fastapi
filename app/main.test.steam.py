...
logging.basicConfig(filename='servico.log', encoding='utf-8', level=logging.INFO)
...
def extract_text_by_sections(pdf_document):
    text_sections = []
    for page_num in range(pdf_document.page_count):  # Itera por cada página do documento PDF
        ...
        for block in blocks:
            ...
            text_sections.append(cleaned_text)  # Adiciona o texto limpo à lista
    return text_sections

# Função para contar tokens
def count_tokens(text: str) -> int:
    tokenizer = tiktoken.get_encoding("cl100k_base")
    return len(tokenizer.encode(text))  # Conta o número de tokens no texto
...
async def process_document(file: UploadFile = File(...)):
    ...
    try:
        ...
        combined_text = " ".join(text_sections)  # Combina todas as seções de texto em uma única string
    ...
    try:
        ...
        db = FAISS.from_documents(chunks, embeddings)  # Cria um índice FAISS para os documentos
        ...
        llm = ChatOpenAI(openai_api_key=openai_api_key, temperature=0.1, model_name="gpt-4-turbo", streaming=True)
        ...
        summary_chunks = []
        async for chunk in llm.astream(messages):  # Corrigido para passar diretamente a lista de mensagens
            if chunk and 'content' in chunk:
                summary_chunks.append(chunk['content'])
                print(chunk['content'], end='')

        # Combina os chunks de resumo em uma única string
